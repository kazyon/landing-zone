...
.DESCRIPTION
  To learn how to use this script, please watch this video: https://youtu.be/9isoxGa4J3A
...
.PARAMETER NewManagementGroupID
    Use this to update the Management Group to a new name during export or import
    This will allow you to move to another tenant with a different Management Group structure / naming
...
.EXAMPLE
  .\Manage-AzureRMBlueprint.ps1 -mode export -NewManagementGroupID "<New Management Group ID>" -ManagementGroupID "<ManagementGroup where Blueprint is located>" -BlueprintName "<MyBlueprint>" -ExportDir "Blueprints"
...
.EXAMPLE
  .\Manage-AzureRMBlueprint.ps1 -mode import "<path of blueprint source> -NewManagementGroupID "<New Target ManagementGroup for Blueprint>" -NewBlueprintName "<New Blueprint Name>"
  This will import a blueprint and artifacts from a source directory and targets a new management group and blueprint name on import
...
(
    ...
    # A new target Management Group Name
    [Parameter(ParameterSetName='Export')]
    [Parameter(ParameterSetName='Import')]
    [string]$NewManagementGroupID,
...
If(!($ManagementGroupID) -and (($mode -eq "Export") -or ($Mode -eq "Report")))
{
...
If($Mode -eq "Export" -or $Mode -eq "Report")
{
    ...
    {
        # Get all Blueprints
        $BlueprintsURI = "https://management.azure.com/providers/Microsoft.Management/managementGroups/$ManagementGroupID/providers/Microsoft.Blueprint/blueprints?api-version=2017-11-11-preview"
        Try
        {
        ...
        {
            write-host "An error occurred - please check rights or parameters for proper configuration and try again" -ForegroundColor Yellow
            write-host "If you recieved " -NoNewline 
            write-host "The access token is invalid." -NoNewline -ForegroundColor Red
            write-host " Please type logout-AzureRMAccount and try again"
            write-host "======================================================================="
    ...
    Else
    {
        ...
        $BluePrintURI = "https://management.azure.com/providers/Microsoft.Management/managementGroups/$ManagementGroupID/providers/Microsoft.Blueprint/blueprints/$BluePrintName" + "?api-version=2017-11-11-preview"
        $ArtifactsURI = "https://management.azure.com/providers/Microsoft.Management/managementGroups/$ManagementGroupID/providers/Microsoft.Blueprint/blueprints/$BluePrintName/artifacts" + "?api-version=2017-11-11-preview"
    #}
    try
    {
    ...
    # Set a folder and update an ID for a New Management Group Name (if necessary)
    If($NewManagementGroupID)
    {
        # Set output folder for export
        $MGFOlder = "MG-$NewManagementGroupID"
        
        # Set new path for MG in Main Blueprint content
        $BlueprintContent.ID = $BlueprintContent.ID -replace "/Microsoft.Management/managementGroups/.*?/", "/Microsoft.Management/managementGroups/$NewManagementGroupID/"
    }
    Else
    {
        $MGFolder = "MG-$ManagementGroupID"
    }
    ...
    if($Mode -eq "Export")
    {
        ...
        IF(!$(Test-Path -Path "$ExportDir\$MGFolder\$TargetBPName"))
        {
            $NewFolder = New-Item -Type Directory "$ExportDir\$MGFolder\$TargetBPName"
        }
        # Exporting main Blueprint
        write-host "Export Folder for export: $ExportDir\$MGFolder\$TargetBPName" 
        ...
        $BlueprintContent|ConvertTo-Json -Depth 50|Out-File "$ExportDir\$MGFOlder\$TargetBPName\Blueprint-$TargetBPName.json"
    }
    ...
    try
    {
        ...
        if($Mode -eq "Export")
        {
            ...
            foreach($Artifact in $Artifacts.value)
            {
                ...
                if($NewBluePrintName)
                {
                    $Artifact.ID = $Artifact.ID -replace "/providers/Microsoft.Blueprint/blueprints/.*?/", "/providers/Microsoft.Blueprint/blueprints/$NewBluePrintName/" 
                }
                if($NewManagementGroupID)
                {
                    # Update Artifact ID for new Management Group name
                    $Artifact.ID = $Artifact.ID -replace "/Microsoft.Management/managementGroups/.*?/", "/Microsoft.Management/managementGroups/$NewManagementGroupID/"
                }
                Write-Host "Exporting Artifact: " -NoNewline -ForegroundColor Cyan
                write-host "$Kind-$Name.json" -ForegroundColor Yellow
                $Artifact|ConvertTo-Json -Depth 50|Out-File "$ExportDir\$MGFOlder\$TargetBPName\$Kind-$Name.json"
            }
        }
...
If($Mode -eq "Import")
{
    ...
    if ($Force -OR $PSCmdlet.ShouldContinue("This operation will attempt to import Blueprint from $ImportDir into your environment. Continue?",$ImportDir) )
    {
        ...
        {
            $FileContent = Get-Content -Path $File.pspath|ConvertFrom-Json
            if($FileContent.type -eq "Microsoft.Blueprint/blueprints/versions" -or $FileContent.Type -eq "Microsoft.Blueprint/blueprints")
            {
                if($NewBluePrintName)
                {
                    # Only supported on draft (non versioned blueprint exports)
                    $FileContent.Name = $NewBluePrintName
                    $FileContent.ID = $FileContent.ID -replace "/providers/Microsoft.Blueprint/blueprints/.*", "/providers/Microsoft.Blueprint/blueprints/$NewBluePrintName"
                }
                if($NewManagementGroupID)
                {
                    $FileContent.ID = $FileContent.ID -replace "/Microsoft.Management/managementGroups/.*?/", "/Microsoft.Management/managementGroups/$NewManagementGroupID/"
                }
            }
            ...
            $ImportURI = "https://management.azure.com/$($FileContent.ID)?api-version=2017-11-11-preview"
            $Body = $FileContent|ConvertTo-Json -depth 50 -Compress
